...
const ManageProfile = () => {
  ...
  const [firstName, setFirstName] = useState(
    initState.auth && initState.auth.user.username.split(" ")[0]
  );
  const [lastName, setLastName] = useState(
    initState.auth && initState.auth.user.username.split(" ")[1]
  );
  ...
  const handleSubmit = (e) => {
    ...
    if (window.confirm("Are you sure you wanna update details?")) {
      axios
        .put(`${process.env.REACT_APP_VAR_API}students/${initState.auth._id}`, {
          ...state,
        })
        .then(async (r) => {
              ...
              <div className="rounded-full flex justify-end gap-3 px-4">
                ...
                <div className="flex flex-col items-end flex-grow">
                  ...
                  <span className="text-gray-500 ">
                    Course: {state.user && state.course.name}
                  </span>
                </div>
